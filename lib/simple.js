...
export function makeWASocket(connectionOptions, options = {}) {
    const LIB_VERBOSE = (process.env.LIB_VERBOSE === 'true') || false
    const vlog = (...args) => { if (LIB_VERBOSE) console.log(...args) }
    vlog('🔍 makeWASocket llamado con opciones:', Object.keys(connectionOptions || {}))
    
    // Verificar si hay propiedades problemáticas
    const problematicProps = ['protocol', 'hostname', 'port']
    const foundProblems = problematicProps.filter(prop => connectionOptions?.hasOwnProperty(prop))
    if (foundProblems.length > 0) {
        console.warn('⚠️ Propiedades problemáticas detectadas:', foundProblems)
    }
    ...
    let conn
    try {
    vlog('🔧 Llamando a _makeWASocket...')
        conn = (global.opts['legacy'] ? makeWALegacySocket : _makeWASocket)(connectionOptions)
    vlog('✅ _makeWASocket exitoso')
    } catch (error) {
        console.error('❌ Error en _makeWASocket:', error.message)
    ...
    return (this?.fromMe || areJidsSameUser(this.conn?.user.id, this.sender)) && this.id.startsWith('3EB0') && (this.id.length === 20 || this.id.length === 22 || this.id.length === 12) || false
        ...
        quoted: {
            get() {
                ...
                const isInteractive = this.mtype.includes('interactive') || this.mtype.includes('Response');
                if (isInteractive && msg && typeof msg === 'object') {
                    console.log('🔍 [TEXT PARSER] Interactive message:', {
                        mtype: this.mtype,
                ...
                if (this.mtype === 'interactiveResponseMessage') {
                    console.log('📱 [PARSER] Detected interactiveResponseMessage');
                    const interactiveResponse = msg?.nativeFlowResponseMessage?.paramsJson
                    if (interactiveResponse) {
                        try {
                            const params = JSON.parse(interactiveResponse)
                            if (params.id) {
                                
                                if (!this._loggedButtonIds?.has(params.id)) {
                                    this._loggedButtonIds = this._loggedButtonIds || new Set();
                                    this._loggedButtonIds.add(params.id);
                                    console.log(`✅ [PARSER] Found button ID: ${params.id}`);
                                }
                                return params.id
                            }
                        } catch (e) {
                            console.error('❌ [PARSER] Error parsing nativeFlowResponse:', e)
                        }
                    }
                }
                ...
                if (isInteractive) {
                    console.log(`📱 [PARSER] Interactive message type: ${this.mtype}`);
                }
                ...
                const finalText = typeof this._text === 'string' ? this._text : '' || (typeof text === 'string' ? text : (
