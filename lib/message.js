...
export async function smsg(client, m, store) {
  store = store || global.store || client.store || { contacts: {}, loadMessage: async () => null }
  ...
  if (m.message) {
    ...
    m.body =
      ...
      m.msg?.buttonResponseMessage?.selectedButtonId ||
    ...
    m.text =
      ...
      m.msg?.title ||
      ''
    const idBot = (client?.user?.jid || (client?.user?.lid ? client.user.lid + '@s.whatsapp.net' : null))
  ...
  client.getName = (jid, withoutContact = false) => {
    ...
    if (id.endsWith('@g.us'))
      return new Promise(async (resolve) => {
        ...
        resolve(
          v.name ||
            v.subject ||
            (id ? id.replace('@s.whatsapp.net', '') : id),
        )
      })
    else
      v =
    ...
    return (
      ...
      v.subject ||
      v.verifiedName ||
      (jid ? jid.replace('@s.whatsapp.net', '') : jid)
    )
  }
  ...
  client.sendButton = async (
    ...
    const botId = (client?.user?.jid || (client?.user?.lid ? client.user.lid + '@s.whatsapp.net' : null))
    const botSettings = global.db.data.settings[botId] || {}
    const botname = botSettings.botname || ''
    const botname2 = botSettings.namebot || ''
    const icon = botSettings.banner || ''
    const dev = global.owner?.[0] ? `${global.owner[0]}@s.whatsapp.net` : 'Desconocido'
    const redes = botSettings.link || 'https://github.com/Brauliovh3'
    const canales = global.miku ? Object.entries(global.miku)
...
const canalSeleccionado = canales.length > 0 ? canales[Math.floor(Math.random() * canales.length)] : { id: null, nombre: null }
  ...
  client.newsletterMsg = async (key, content = {}, timeout = 5000) => {
    ...
    const botIds = (client?.user?.jid || (client?.user?.lid ? client.user.lid + '@s.whatsapp.net' : null))
  ...
  client.sendContextInfoIndex = async (
    ...
    const sourceUrl = typeof config.redes === 'string' ? config.redes : typeof settings.link === 'string' ? settings.link : 'https://github.com/Brauliovh3'
