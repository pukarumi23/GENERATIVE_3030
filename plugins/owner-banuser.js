var handler = async (m, { conn, text, usedPrefix, command }) => {
    ...
    try {
        ...
        number = user.split('@')[0]
        if (user === conn.user.jid) return conn.reply(m.chat, `💙 @${bot} No puede ser baneado con este comando.`, m, rcanal,{ mentions: [user] })
        ...
        if (users[user].banned === true) return conn.reply(m.chat, `💙 No es necesario volver a banear a @${number}.`, m, rcanal, { mentions: [user] })
