...
export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!value && !m.quoted && !m.message.imageMessage)
      return m.reply('💙 Debes enviar o citar una imagen para cambiar el icon del bot.', m, global.miku)
    if (value.startsWith('http')) {
      config.icon = value
      return m.reply(`💙 Se ha actualizado el icon de *${config.namebot}*!`, m, global.miku)
    }
