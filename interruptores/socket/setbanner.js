...
export default {
  ...
  return m.reply('💙 Debes enviar o citar una imagen o video para cambiar el banner del bot.', m, global.miku)
    if (value.startsWith('http')) {
      config.banner = value
      return m.reply(`💙 Se ha actualizado el banner de *${config.namebot}*!`, m, global.miku)
    }
    const q = m.quoted ? m.quoted : m.message.imageMessage ? m : m
    const mime = (q.msg || q).mimetype || q.mediaType || ''
    if (!/image\/(png|jpe?g|gif)|video\/mp4/.test(mime))
      return m.reply('💙 Responde a una imagen válida.', m, global.miku)
    const buffer = await q.download()
    if (!buffer) return m.reply('💙 No se pudo descargar la imagen.', m, global.miku)
