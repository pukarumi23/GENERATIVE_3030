...
const captions = {
  ...
  slap: (from, to, genero) => from === to ? `🔶 se dio una bofetada a sí ${genero === 'Hombre' ? 'mismo' : genero === 'Mujer' ? 'misma' : 'mismx'}.` : '🏵️ le dio una bofetada a',
  bath: (from, to) => (from === to ? '🧡 se está bañando.' : '🧡 está bañando a'),
  angry: (from, to, genero) => from === to ? `🏵️ está muy ${genero === 'Hombre' ? 'enojado' : genero === 'Mujer' ? 'enojada' : 'enojadx'}.` : `🏵️ está super ${genero === 'Hombre' ? 'enojado' : genero === 'Mujer' ? 'enojada' : 'enojadx'} con`,
  ...
  bite: (from, to, genero) => from === to ? `🏵️ se mordió ${genero === 'Hombre' ? 'solito' : genero === 'Mujer' ? 'solita' : 'solitx'}.` : '🏵️ mordió a',
  bleh: (from, to) => from === to ? '🧡 se sacó la lengua frente al espejo.' : '🧡 le está haciendo muecas con la lengua a',
  bonk: (from, to, genero) => from === to ? `🏵️ se dio un bonk a sí ${genero === 'Hombre' ? 'mismo' : genero === 'Mujer' ? 'misma' : 'mismx'}.` : '🏵️ le dio un golpe a',
  blush: (from, to) => (from === to ? '🧡 se sonrojó.' : '🧡 se sonrojó por'),
  impregnate: (from, to) => (from === to ? '🏵️ se embarazó.' : '🏵️ embarazó a'),
  ...
  cry: (from, to) => (from === to ? '🏵️ está llorando.' : '🏵️ está llorando por'),
  happy: (from, to) => (from === to ? '🧡 está feliz.' : '🧡 está feliz con'),
  coffee: (from, to) => (from === to ? '🏵️ está tomando café.' : '🏵️ está tomando café con'),
  clap: (from, to) => (from === to ? '🧡 está aplaudiendo por algo.' : '🧡 está aplaudiendo por'),
  cringe: (from, to) => (from === to ? '🏵️ siente cringe.' : '🏵️ siente cringe por'),
  dance: (from, to) => (from === to ? '🧡 está bailando.' : '🧡 está bailando con'),
  cuddle: (from, to, genero) => from === to ? `🏵️ se acurrucó ${genero === 'Hombre' ? 'solo' : genero === 'Mujer' ? 'sola' : 'solx'}.` : '🏵️ se acurrucó con',
  ...
  dramatic: (from, to) => from === to ? '🏵️ está haciendo un drama exagerado.' : '🏵️ le está haciendo un drama a',
