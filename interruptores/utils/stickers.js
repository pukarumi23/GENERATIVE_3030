...
export default {
  ...
  run: async (client, m, args) => {
    ...
    if (!raw) return m.reply('💙 Por favor, ingresa el nombre de los *stickers* a buscar.', m, global.miku)
    ...
    try {
      ...
      if (!packs.length) return m.reply('💙 No se encontraron resultados.', m, global.miku)

      const pack = packs[0]
      const stickerUrls = await getStickers(pack.url)
      if (!stickerUrls.length) return m.reply('💙 El pack no contiene stickers.', m, global.miku)
...
const parseQueryAndMeta = (raw, m) => {
  ...
  return {
    query: parts[0],
    packname: parts[1] || user.metadatos || "💙HATSUNE MIKU💙",
